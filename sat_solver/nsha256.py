...
"""
This is just the normal SHA-256 algorithm, for reference.
SHA-256 hash has 32 bytes (64 hexadecimal characters, 256 bits)
"""
...
def as_bin_str(bytes_obj):
  s = ''
  for b in bytes_obj:
    x = bin(int(str(b)))[2:]
    x = ('0' * (8 - len(x))) + x
    s += x
  assert len(s) == len(bytes_obj) * 8
  return s
...
def pad(msglen):
  ...
  result = b'\x80' + (b'\x00' * padlen) + length
  return result
...
class SHA256:
  ...
  def digest(self):
    self.update(pad(self._counter))

    # '!L' means big-endian, 4 bytes
