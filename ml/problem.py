...
class AndGate(object):
  def __init__(self, out, inp1, inp2):
    ...
    assert 0 not in (out, inp1, inp2), 'Bad AND gate: (%d, %d, %d)' % (out, inp1, inp2)
...
class Problem(object):
  def __init__(self, input_size, output_size, num_vars, num_gates, num_clauses,
    ...
    assert type(self.input_size) is int, 'Not all properties initialized'
    assert type(self.output_size) is int, 'Not all properties initialized'
    assert type(self.num_vars) is int, 'Not all properties initialized'
    assert type(self.num_gates) is int, 'Not all properties initialized'
    assert type(self.num_clauses) is int, 'Not all properties initialized'
    assert type(self.input_indices) is list, 'Not all properties initialized'
    assert type(self.output_indices) is list, 'Not all properties initialized'
    assert type(self.cnf_indices) is list, 'Not all properties initialized'
  ...
  def parse_dimacs(filename, num_vars, skip_assignments=True):
    ...
    with open(filename, 'r') as f:
      for line in f:
        ...
        for x in lits:
          xa = abs(x)
          assert 0 < xa <= num_vars, 'Invalid CNF, line %d: %s' % (line_no, line)
          if x < 0:
