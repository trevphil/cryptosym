...
Probability::Probability(std::shared_ptr<Dataset> dataset, std::shared_ptr<utils::Config> config)
...
double Probability::probOne(size_t rv_index,
  ...
  if (algorithm == "cpd") {
    ...
    if (num_observed < observed_neighbors.size()) {
      spdlog::warn("Took subset of {}/{} observed neighbors for RV {}", num_observed,
                   observed_neighbors.size(), rv_index);
    }
  } else {
